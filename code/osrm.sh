...
Example:
    zsh osrm.sh msa 12060-atlanta drive
...
- [region_id]: Required
    ID of the region, given by the syntax '{FIPS code}-{friendly-name}'.
...
- [mode]: Required
    Mode of travel. The server initialization requires setting up routing for 
...
- [port=5108]
    Port number on which the local server will listen.

- [nmax=3000]
    Maximum no. of rows in the OD table that the server can process in one 
